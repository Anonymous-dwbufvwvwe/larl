...
class LLM_Net(nn.Module):
    def __init__(self, model_name="HuggingFaceTB/SmolLM2-135M-Instruct", frozen_layer_num=-1, hidden_dim=256):
        super().__init__()

        self.device = torch.device("cuda:0")
               
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.llm = AutoModelForCausalLM.from_pretrained(model_name, output_hidden_states=True).to(self.device)
    ...
    def set_trainable_layers(self):
        for name, param in self.llm.named_parameters():
            if "model.layers.29" in name:
                param.requires_grad = True
            else:
                param.requires_grad = False
                
    def forward(self,obs_batch,if_customerize=False):
        prompts = []

        for single_obs in obs_batch:
            current_scenario = build_single_scenario_prompt(single_obs)
            prompt = build_prompt(current_scenario)
        ...
        latent = (last_hidden * attention_mask).sum(dim=1) / attention_mask.sum(dim=1)
...
class BasePolicy(BaseModel, ABC):
    ...
    def predict(
        ...
        with th.no_grad():
            actions = self._predict(obs_tensor, deterministic, cus)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        """
        self.llm_net = LLM_Net()

        latent_dim_pi = self.llm_net.llm.config.hidden_size
        ...
        self.value_net = nn.Linear(latent_dim_pi, 1)
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        latent_pi, latent_vf = self.llm_net(obs)        
    ...
    def _predict(self, observation: PyTorchObs, deterministic: bool = False, cus: bool = False) -> th.Tensor:
        """
        Get the action according to the policy for a given observation.
        ...
        :return: Taken action according to the policy
        """
        action = self.get_distribution(observation,cus).get_actions(deterministic=deterministic)
        return action
    ...
    def get_distribution(self, obs: PyTorchObs, cus: bool = False) -> Distribution:
        """
        ...
        """
        latent = self.llm_net(obs,cus)
        return self._get_action_dist_from_latent(latent[0])
    ...
    def predict_values(self, obs: PyTorchObs) -> th.Tensor:
        ...
        """
        latent = self.llm_net(obs)
        return self.value_net(latent[0])
